...
class undname_creator_t(object):
    ...
    def format_var(self, declaration, hint):
        result = []
        is_mem_var = isinstance(declaration.parent, declarations.class_t)
        if is_mem_var and declaration.type_qualifiers.has_static and hint == 'msvc':
            result.append('static ')
        if hint == 'msvc':
            result.append(
                self.__format_type_as_undecorated(declaration.decl_type, False, hint))
            result.append(' ')
        if is_mem_var:
            result.append(
                self.__remove_leading_scope(declaration.parent.decl_string) + '::')
        result.append(declaration.name)
        return ''.join(result)

    def format_decl(self, declaration, hint=None):
        ...
        if isinstance(declaration, declarations.calldef_t):
            name = self.format_calldef(declaration, hint)
        elif isinstance(declaration, declarations.variable_t):
            name = self.format_var(declaration, hint)
        else:
