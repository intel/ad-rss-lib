...
to break. According to the RSS papers, this should always result
...
its own lane can break with lower force than the car on the opposite direction.
...
on the wrong lane. Therefore, each car will break with low force and expects
the other car to break more strongly. Consequently, this dangerous situation
...
will extend the rule given by the paper to the following.
 ...
 1. In case that one car is on the correct lane and the other car on the lane
 belonging to the first car and there is a lateral conflict. If there is
no  longitudinal conflict if both cars break according to the stated
breaking pattern, but the longitudinal distance is not safe according to the
"Safe Longitudinal Distance for Opposite Directions", given in the paper,
the car on the correct lane is allowed to break with the lower breaking force,
whether the other car must break according to the stated breaking pattern.
(This is the situation described in the RSS paper).

  2. Given a situation that there are two cars approaching each other with
  lateral conflict  and  the longitudinal distance is no longer safe,
  if both cars accelerate at maximum   during their response time and break with
  minimum required breaking force.
  Both cars must break according to the stated breaking pattern.
...
The assumption that a car can always accelerate at &alpha;~𝑎𝑐𝑐𝑒𝑙,𝑚𝑎𝑥~
