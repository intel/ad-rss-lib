 ...
 *       ::ad::physics::Acceleration(-1e2) <= brake_min < ::ad::physics::Acceleration(0.)
 */
                                  ...
                                  bool const logErrors = true)
{
  ...
  inValidInputRange
    = withinValidInputRange(input.accel_max, logErrors) && withinValidInputRange(input.brake_min, logErrors);
  ...
  {
    inValidInputRange
      = (::ad::physics::Acceleration(-1e2) <= input.brake_min) && (input.brake_min < ::ad::physics::Acceleration(0.));
    if (!inValidInputRange && logErrors)
    {
      spdlog::error("withinValidInputRange(::ad::rss::world::LateralRssAccelerationValues)>> {} element {} out of "
                    "valid input range [{}, {}]",
