...
inline bool withinValidInputRange(::ad::rss::state::RssStateInformation const &input, bool const logErrors = true)
{
  ...
  inValidInputRange = withinValidInputRange(input.safe_distance, logErrors)
    && withinValidInputRange(input.current_distance, logErrors) && withinValidInputRange(input.evaluator, logErrors);
